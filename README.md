...
| Anthropic 订阅配额查询 + 60s/429 backoff 缓存 | `src/account_quota.py` `AccountQuotaService` |
...
# 读取订阅配额缓存（5h/7d/Opus/Sonnet 滚动窗口使用率，不打上游）
curl -s -H "Authorization: Bearer $KEY" http://127.0.0.1:18787/admin/quota | jq

# 强制刷新订阅配额（向 Anthropic 拉一次；服务端 60s 频控 + 429 backoff 5min）
curl -X POST -H "Authorization: Bearer $KEY" http://127.0.0.1:18787/admin/quota/refresh
```
...
{
  "ok": true,
  "snapshot": {                            // 上次成功结果；429 之类失败不会覆盖
    "fetched_at_unix": 1747890123.5,
    "five_hour":        {"utilization": 42.0, "resets_at": "2026-05-22T18:30:00Z"},
    "seven_day":        {"utilization": 67.0, "resets_at": "2026-05-26T00:00:00Z"},
    "seven_day_opus":   {"utilization": 81.0, "resets_at": "..."},
    "seven_day_sonnet": {"utilization": 34.0, "resets_at": "..."},
    "extra_usage": null                    // 启用按量付费时是金额对象
  },
  "last_error": null,                      // 最近一次失败（成功后清空）
  "rate_limited": false,                   // 本次请求是否被服务端拒（cooldown 期内）
  "rate_limit_seconds": 60.0,              // 普通频控间隔
  "seconds_until_next_refresh": 0.0        // 距下次可刷新还有多久（含 429 backoff）
}
```

**订阅配额查询机制**（`src/account_quota.py`）：

| 项 | 值 |
|---|---|
| 上游 endpoint | `GET https://api.anthropic.com/api/oauth/usage` |
| 鉴权 | operator 的 `~/.claude/.credentials.json` 里的 `claudeAiOauth.accessToken`（OAuthRefresher 保证 fresh） |
| 必需 header | `anthropic-beta: oauth-2025-04-20` |
| 普通频控 | 服务端 `MIN_REFRESH_INTERVAL_SECONDS = 60s` 间隔，期间任何 refresh 请求直接回缓存 |
| 429 backoff | 命中 429 → 解析 `Retry-After`（数字秒），没给则默认 5 分钟 |
| 错误隔离 | 失败**不覆盖** `snapshot`，只写 `last_error`——dashboard 不会因为一次 429 丢掉历史可见数据 |
| 启动行为 | 不主动预拉。首次 GET 缓存为空，前端/运维需手动 POST `/admin/quota/refresh` |

---

### 2.11 监控面板 `/ui`

浏览器打开 `http://YOUR-HOST:18787/ui` —— 一个内嵌的深色单页面板，分四个区块：

| 区块 | 内容 |
|---|---|
| **顶部 app bar** | brand + API Key 输入 + 全局动作（重新加载 config / 刷新 OAuth 凭据） |
| **系统概览** | CLI 版本、存活 worker、活跃数、异常数（4 张 stat 卡片，左侧色条区分） |
| **订阅配额** | 5h/7d/Opus/Sonnet 各自一个进度条；按使用率自动变色（绿 < 70% < 黄 < 90% < 红）；右上「刷新配额」按钮带服务端 60s 频控倒计时 |
| **用户池拓扑** | 按 token 分组的 user 池（空池整段隐藏） |
| **Worker 运行状态** | 每个 worker 一行：用户 / 状态徽章 / 模型 / LiteLLM 用户 / 运行+空闲时长 / 累计请求 / 进行中 / 当前任务（preview hover 显完整文本）/ 重启按钮 |
