...
> 隐患：多 worker 撞同一秒过期会产生 OAuth refresh_token 并发冲突 ——
> A 用 RT0 刷成功拿 RT1，B 用 RT0 去刷收 `invalid_grant`。这是协议层的
> 竞态，跟"目录共享"是两个正交问题，目前未处理。
...
| 想看什么 | 文件:行 |
|---|---|
| body 合并规则 | `src/mitm/addon.py:136` `_merge_body` |
| system 字段合并 | `src/mitm/addon.py:189` `_merge_system` |
| 响应流 tap | `src/mitm/addon.py:231` `responseheaders` / `_tap` |
| 用户可覆盖的 body 字段白名单 | `src/mitm/addon.py:29` `USER_OWNED_BODY_FIELDS` |
...
| 每用户 HOME seed（共享 `.claude/` 目录） | `src/session/session.py:88` `_seed_home` |
| 主进程 session：spawn worker + 读 stdout | `src/session/session.py` |
| user 池负载均衡 | `src/session/manager.py:41` `pick` |
| SSE → 完整 Message 重组（含 error 兜底） | `src/api/anthropic.py:41` `_collapse_stream` |
| OpenAI ↔ Anthropic 字段转换 | `src/api/translate.py` |
| token → user 池 | `src/auth.py` + `src/config.py` |
...
claude:
  ...
  restart_interval_seconds: 43200   # 12h: worker 周期性就地重启清掉 CLI 状态
...
| `claude.restart_interval_seconds` | 定时重启间隔。worker 不会被 idle 回收；到达 age 后**就地重启**（同 session、同端口），用来清掉 CLI 累积状态。优雅等待最多 60s 让在飞请求结束。默认 43200（12h）。 |
| `users` | `bearer_token → user_id` 映射；值可以是字符串（单 worker）或字符串列表（pool，自动负载均衡）。 |
...
- **所有 user 共享一个订阅配额**（同一个 OAuth 账号）；池只解决本服务的
  worker 并行度，并不放大 Anthropic 端的限速
- 多账号池 / 配额追踪不在当前实现范围内

### 3.5 dev 模式（不走 Docker）
