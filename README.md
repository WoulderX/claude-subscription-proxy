...
### 1.3 给新手：mitm 劫持原理（从零讲起）

如果你对 HTTPS / TLS / 代理这些底层概念不熟，先看这一节再读 §1.4 的代码细节。本节回答一个核心问题：

> **HTTPS 设计目的就是"防止中间人偷看和篡改"，那我们这套方案为什么能在 claude CLI 和 Anthropic 之间插一个 mitmproxy 改请求？**

#### 普通 HTTPS 默认就拦不了

claude CLI 直发 `https://api.anthropic.com/v1/messages` 时的物理过程：

1. **DNS**：查到 api.anthropic.com 的 IP（比如 160.79.104.10）
2. **TCP 三次握手**：跟 160.79.104.10:443 建立连接
3. **TLS 握手（关键）**：
   - 客户端发 ClientHello（带 SNI: api.anthropic.com，明文字段）
   - 服务器回 ServerHello + **证书**（subject = api.anthropic.com，issuer = DigiCert，由 DigiCert 私钥签名）
   - 客户端检查：subject 对得上 ✓ / issuer 在我信任的 CA 列表 ✓ / 用 DigiCert 公钥验签通过 ✓
   - 握手通过，协商出对称加密密钥
4. **加密通信**：之后所有 HTTP 字节都用这个密钥加密

**为什么默认拦不了**：第 3 步检查证书时，客户端只信任**系统内置或受信 CA 签发**的证书。任何想冒充 api.anthropic.com 的中间人都拿不到 DigiCert 的私钥，签不出能验证通过的证书 → 握手失败。

#### mitmproxy 的破解招数：让客户端先信任假 CA

mitmproxy 的核心做法是**让客户端把 mitmproxy 自己的 CA 加进可信列表**：

1. mitmproxy 第一次运行时自己生成一对 RSA 密钥，签发一张 "mitmproxy CA" 证书
   - 公钥证书：`~/.mitmproxy/mitmproxy-ca-cert.pem`
   - 私钥严密保存
2. 通过环境变量告诉客户端"信任这个 CA"：

```python
# src/pty_driver.py:42
env["NODE_EXTRA_CA_CERTS"] = str(self.ca_cert)   # mitmproxy-ca-cert.pem
```

Node 看到这个变量后，把 mitmproxy CA 加进可信列表 —— 跟 DigiCert / Let's Encrypt **平起平坐**。

魔法就成立了：

- mitmproxy 持有该 CA 私钥 → **能用它签发任意域名的证书**
- 客户端信任该 CA → **接受 mitmproxy 签发的任意证书**

#### 流量怎么走到 mitm：HTTPS_PROXY + CONNECT 隧道

光有信任还不够，得让 claude CLI 的流量**物理上**经过 mitmproxy：

```python
# src/pty_driver.py:40-41
env["HTTPS_PROXY"] = "http://127.0.0.1:18000"
env["HTTP_PROXY"]  = "http://127.0.0.1:18000"
```

Node 内置 HTTP 客户端看到这个变量后，**所有出站请求都先发给本机 18000 端口**。

但 HTTPS_PROXY 协议头明明是 `http://`，怎么传 HTTPS？答案是 HTTP 标准的 **`CONNECT`** 方法：

```
Step 1: Node 跟 mitm 建明文 TCP 连接
Step 2: Node 发 → CONNECT api.anthropic.com:443 HTTP/1.1
Step 3: mitm 答 → HTTP/1.1 200 Connection Established
Step 4: 之后这条 TCP 通道被当成"裸 TCP 隧道"用
```

按 HTTP 规范，第 4 步之后代理应该老老实实当透明水管，**但 mitmproxy 不按规矩走**。

#### 偷天换日的瞬间

第 4 步之后 Node 以为自己在跟 api.anthropic.com 直接做 TLS 握手。实际发生的是：

```
Node 发 ClientHello { SNI: api.anthropic.com, ... }
   ↓ 物理上发给 mitmproxy（因为 TCP 隧道是 mitm 在管）

mitmproxy:
  1. 从 ClientHello 读出 SNI = "api.anthropic.com"
  2. 现场用自己 CA 私钥签一张假证书：
       subject: CN=api.anthropic.com
       issuer:  CN=mitmproxy
  3. 把假证书塞进 ServerHello 回给 Node

Node 收到 ServerHello:
  - subject 对得上 ✓
  - issuer (mitmproxy) 在我信任的 CA 列表? ← 在！（NODE_EXTRA_CA_CERTS 加进去了）
  - 验签通过 ✓
  → "好，握手继续"

   ←── Node 和 mitmproxy 完成 TLS 握手（用假证书）──→
   之后 Node 发的 HTTP 请求被加密，但 mitmproxy 持有解密密钥 → 看到明文
```

**与此同时**，mitmproxy 还要跟真的 api.anthropic.com 单独建一条 TLS 连接（这次用真证书、正常 TLS 握手）。最终拓扑：

```
Node   ←TLS(假证书)→   mitmproxy   ←TLS(真证书)→   api.anthropic.com
         明文给 mitm                    明文给 mitm
                       mitm 持两端密钥
                       双向都能看明文 / 改字节
```

mitmproxy 是**两端独立 TLS 会话的端点**，不是简单的字节转发。它在两端中间做的事就是普通 HTTP：读请求、改 body、写出去；读响应、tap 字节、写回去。

#### 整套机制依赖的四件事

```
1. HTTPS_PROXY=http://127.0.0.1:18000   → 让流量物理上经过 mitm
2. NODE_EXTRA_CA_CERTS=mitm-ca.pem      → 让 Node 信任 mitm 假证书
3. mitmproxy 持有该 CA 私钥             → 能现场签任意域名证书
4. mitmproxy 11 的 request/response hook → 在两端中间改 body / tap 字节
```

每一环单独看都很无聊，**凑齐这四个，TLS 加密的安全前提就被打破了**。

注意：这套机制**只对设了上述环境变量的客户端有效**。同一台机器上没设这些变量的程序（比如系统的 `curl`）连接 anthropic.com 仍然会拒绝 mitm 的假证书 —— 没把 mitm CA 加进它的信任列表，TLS 握手过不去。所以"hijack"只在 worker 内的 claude CLI 这条特定路径上发生，不会污染机器上其它 HTTPS 调用。

理解了"为什么能成立"之后，§1.4 看具体怎么写代码：mitmproxy 11 的 `request()` / `responseheaders()` hook 用法、body 合并白名单、响应 tap 函数。

### 1.4 mitm 是怎么劫持 claude 的
